...
def load_single_table(data_dir, verbose=True):
    dataset_meta = json.load(open(os.path.join(data_dir, 'dataset_meta.json'), 'r'))

    # For single table, we have no links between the tables - Loaded tables are all parents
    relation_order = []

    tables = {}

    for table, meta in dataset_meta['tables'].items():
        # Currently, we only want to add the transactions table (Berka) to our data (Single-Table)
        if table != 'trans':
            continue
        # Construct relation_data, taking every table as a parent table
        relation_order.append([None, f"{table}"])
        tables[table] = {
        ...
        # Get all the columns that have '_id' in the col name
        id_cols = [col for col in tables[table]['df'].columns if '_id' in col]
        # Create a df without any of id columns
        df_no_id = tables[table]['df'].drop(columns=id_cols)
        info = get_info_from_domain(
            df_no_id,
            tables[table]['domain']
        )
        # TODO: Verify what this function does - use on raw or preprocessed data?
